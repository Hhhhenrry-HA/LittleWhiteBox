...
import { buildSdImageRequest, compile as compileSdScenePlan, SD_REQUEST_DELAY_MS } from "./compiler.js";
import {
...
async function refreshSingleImage(container) {
    try { await redrawImageCard("sdwebui", container); }
...
async function runPreparedSdSlots(input) {
    ...
    try {
        ...
        const recipe = createSdGenerationRecipe({
            settings,
            characterTags: getSharedDrawSettings().characterTags || [],
...
export async function initSdDraw() {
    ...
    preparedImageDispose = registerPreparedImageProvider("sdwebui", createImageCardRedrawProvider({
