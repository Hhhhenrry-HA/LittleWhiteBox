...
test('requests accept only capability IDs and difficulty; all invalid input is rejected before sampling', () => {
    const invalid = [{ ...skill, value: 65 }, { ...skill, kind: 'skill' }, { ...skill, opponent: {} },
        ...
        { ...skill, stat: 'Climb' }, { ...skill, stat: 'hpMax' }, { ...skill, stat: '__proto__' },
...
test('model capabilities include base-only skills, never values; the injected example really executes', () => {
    ...
    for (const item of capabilities) assert.deepEqual(Object.keys(item).sort(), ['id', 'name', 'use']);
    const prompt = buildActionCheckPrompt('', [], 'standard', 'coc7', true);
    const lines = prompt.split('\n');
    assert.deepEqual(JSON.parse(lines.find(line => line.startsWith('[{'))), capabilities);
    const start = prompt.indexOf('<xb_action_check>{');
    const end = prompt.indexOf('</xb_action_check>', start) + '</xb_action_check>'.length;
    assert.equal(prepareActionCheck({ body: prompt.slice(start, end), rule: 'coc7', coc7Sheet: sheet(), generatedFrom: 0, id: 'example' }).kind, 'candidate');
