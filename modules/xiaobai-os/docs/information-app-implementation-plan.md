...
上传先于消息保存；上传失败/切聊取消不得写消息或调用回复模型，当前页面保留草稿；未确认保存继续走已有 Kernel 恢复。已存消息重试不重新上传。原图引用只允许上述本地目录与固定 hash 文件名，模型附件字段被拒绝，不加载任意 URL。查看原图不依赖画图开关，不生成新图。私人信息 XML 记录 `附件` 路径并纳入投影摘要，原生楼层可展示真实图片，但不把像素塞入剧情文本。

本轮与未压缩的本线程历史图片读回后以多模态内容传给回复模型；旧图压缩时也交给摘要模型。图像读取失败明确中止，不静默降为文件名；需要所选 Agent 模型支持看图。每张原图占用 6000 个上下文预算单位（不是 token 估算），沿用 18000/8000/16000 的压缩预算，防止纯图片对话无限携图。临时 data URL 和发送草稿不持久化；保存确认只清除同一图片及配文，不清新草稿。
