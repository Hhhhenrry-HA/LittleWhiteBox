...
「维护一次」只处理聊天尾部最新的完整 User + 当前所选 Assistant 内容；这次明确点击不要求自动维护已开启。没有完整轮或正在生成时只显示本地提示，不调用 API。「从当前聊天建立/重建地图」才允许扫描更长历史并以完整候选 Map 一次替换旧 Map，两者不能共用含糊按钮。点击后直接交给 Host 后台队列，不弹确认框；按钮立即禁用并显示对应运行状态。
...
关键交互包括：场景/世界切换、地点选择、缩放、拖拽、复位、图例、空状态、自动维护开关、明确的维护状态和显式维护/重建按钮。维护中只由原按钮显示“正在维护/正在重建”并禁用，不增加等待弹窗。打开后首屏不能因 host 请求留出空白顶部，底部导航沿用 OS 黑色 token。
