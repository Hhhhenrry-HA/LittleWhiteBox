...
定位、教学循环、书桌/课堂交互及用户级资产与聊天级奖励的边界见 [语言学习 APP 产品设计](./language-learning-app-target-design.md)。终态覆盖听说读写，复用现有 TTS 合成与播放器，分别定义录音、转写与音频评价；材料形态、回答方式与学习证据分开，不先固化成纯文本聊天。当前仅落设计，尚未实现或注册入口；用户级存储、Tavily 正文获取、媒体与奖励接入须在施工前验证，不能套用现有单 sidecar 原子事务假设。
