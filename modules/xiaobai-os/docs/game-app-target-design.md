...
终态是「游艺室 → 玩法房间 → 结算」，不是大厅堆三份下注表单。大厅完全由目录驱动，提供搜索、分类和当前局入口；三个或三十个游戏采用同一布局。浏览不触发动作，一局没结束时其他房间仍能看说明，但只能返回当前局继续。
...
- 视觉沿用 OS 基础字体与主题，游艺室纸白、骰桌深绿、牌桌酒红、阶梯暖金；本地 SVG/CSS，无素材网络、依赖或新持久状态。API / 四次元壁内部不改。
