...
- 当时未给地图、钱包、银行、赌场、任务和宠物创建占位入口；钱包在独立的 Economy 阶段完成后才注册。
...
- 钱包只是 Economy 的只读投影，不拥有余额写入，也不承载任务、商店、银行、赌场或宠物逻辑。
...
Economy 的剧情指纹适配由 host 提供，账本冻结、对账、回滚与写入门归`domains/economy`所有；钱包只读取投影。当前已验证编辑、swipe、尾删、中间单删、切聊和真实分支回滚，桌面及 390×844 移动布局均可用。真实生成请求的错误恢复也已复验：供应商返回 HTTP 错误时只显示清晰告警，不把错误 JSON 持久化或伪装成 AI 回复。

下一项业务 APP 尚未开工。任务、商店、银行/赌场、宠物或地图进入前，必须先完成自己的终态边界，再作为 Economy 的真实消费者接入。
