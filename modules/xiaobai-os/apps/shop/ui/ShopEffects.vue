...
<template>
    <section class="shop-page" aria-labelledby="shop-effects-title">
        <header class="shop-page-heading"><span class="shop-eyebrow">故事里的小小变化</span><h2 id="shop-effects-title">生效中 <small>{{ active.length }} 个</small></h2><p>这些奇物的效果已启用，将按各自规则影响后续回复。</p></header>
        ...
        <div v-if="active.length" class="shop-effect-list">
            <article v-for="activation in active" :key="activation.activationId" class="shop-effect-card">
                <header><ShopItemArt :name="activation.icon" /><div><span class="shop-effect-indicator"><i />已启用</span><h3>{{ activation.name }}</h3><strong>{{ activation.stateLabel }}</strong></div></header>
        ...
        <div v-else class="shop-empty"><span><ShopIcon name="spark" /></span><h3>故事正按自己的节奏继续</h3><p>使用背包中的奇物后，可以在这里查看效果。</p><button type="button" class="shop-secondary-button" @click="$emit('inventory')">打开背包</button></div>
        ...
        <p class="shop-footnote">有限效果按新回复消耗次数。<br>关闭或用完，不会撤销已经发生的剧情。</p>
    </section>
</template>
