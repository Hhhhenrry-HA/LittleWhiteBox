...
// Drafts have every score from the start, but their budgets need not be spent yet.
...
export function emptyCoc7Draft(): Coc7Draft {
    return { attributes: allocation('attributes', () => COC7_POINTS.min), skills: allocation('skills', () => COC7_POINTS.min) } as Coc7Draft;
}
export function canAdjustCoc7Stat(draft: Coc7Draft, stat: Coc7Stat, direction: -1 | 1): boolean {
    ...
    return (direction === -1 || direction === 1) && next >= COC7_POINTS.min && next <= COC7_POINTS.max
