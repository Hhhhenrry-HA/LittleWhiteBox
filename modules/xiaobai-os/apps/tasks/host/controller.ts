...
import type { MaintenanceRunOutcome, MaintenanceRunner } from '../../../host/maintenance/runner.js';
...
interface TaskControllerDependencies {
    tasks: TasksService;
    economy: Pick<EconomyRepository, 'hasCurrent' | 'ensureCurrent'>;
...
function publicError(error: unknown): Error {
    ...
    if (code === 'SAVE_UNCONFIRMED') {return new Error('tasks_save_unconfirmed');}
    if (code === 'SAVE_CONFLICT') {return new Error('tasks_save_conflict');}
    if (code === 'CHAT_CHANGED') {return new Error('tasks_chat_changed');}
...
export function createTaskController({
...
}: TaskControllerDependencies): XiaobaiOsAppRuntime & {
    ...
    function serviceView(): TasksServiceView {
        if (economy.hasCurrent()) {return tasks.readCurrent();}
        return {
            domain: null,
            records: [],
            playerBalance: 0,
    ...
    function schedulePreparation(current: TaskActivation): void {
        ...
        globalThis.setTimeout(() => {
            ...
            void economy.ensureCurrent().then(() => {
                if (preparation !== pending || activation !== current || currentChatIdentity() !== current.chatIdentity) {return;}
                preparation = null;
                emitState(current);
            }).catch((error) => {
    ...
    function assertWritable(current: TaskActivation): void {
        ...
        if (!economy.hasCurrent() || activation !== current || currentChatIdentity() !== current.chatIdentity) {
