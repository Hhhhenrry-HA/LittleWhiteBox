...
<template>
    <section class="tasks-page tasks-publish-page">
        <header class="tasks-page-heading"><button type="button" class="tasks-back" :disabled="busy" @click="emit('cancel')">← 返回</button><div><h2>发布任务</h2></div></header>
        <form class="tasks-publish-form" @submit.prevent="submit">
            <aside><small>当前可用余额</small><strong>¤ {{ balance }}</strong><p>发布后，报酬会立即托管。招募期间撤回可全额退回；选定执行者后不可撤回。</p></aside>
            <label><span>任务标题 <b>*</b></span><input v-model="form.title" required maxlength="120" autocomplete="off" placeholder="一句清楚的任务名称"></label>
            ...
            <footer><button type="button" :disabled="busy" @click="emit('cancel')">取消</button><button type="submit" class="tasks-primary-button" :disabled="busy || Boolean(disabledReason) || form.reward > balance" :title="disabledReason">{{ busy ? '正在发布…' : '确认托管并发布' }}</button></footer>
        </form>
    </section>
</template>
