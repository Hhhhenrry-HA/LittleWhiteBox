...
defineProps<{ task: TaskRecord; sourceLabel?: string }>();
...
<template>
    <button type="button" class="tasks-record" :data-navigation-id="`task:${task.taskId}`" @click="$emit('open', task)">
        ...
        <span class="tasks-record-foot"><span><TaskIcon :name="task.source === 'received' ? 'pin' : 'people'" />{{ sourceLabel || (task.source === 'received' ? task.location : task.assignee?.displayName || `${task.candidates.length} 位候选人`) }}</span><TaskIcon name="next" /></span>
    </button>
</template>
