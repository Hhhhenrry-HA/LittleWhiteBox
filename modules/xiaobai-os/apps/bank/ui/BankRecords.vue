...
<template>
    <section class="bank-page" aria-labelledby="bank-records-title">
        ...
        <div v-else class="bank-record-list">
            <details v-for="activity in activities" :key="bankActivityKey(activity)" class="bank-record-row">
                ...
                <dl class="bank-record-detail"><div><dt>投入本金</dt><dd>¤ {{ activity.amountIn.toLocaleString('zh-CN') }}</dd></div><div><dt>实际到账</dt><dd>¤ {{ activity.payout.toLocaleString('zh-CN') }}</dd></div><div><dt>结算回合</dt><dd>{{ activity.turnLabel }}</dd></div><div><dt>发生时间</dt><dd>{{ dateFormatter.format(activity.createdAt) }}</dd></div><div v-if="activity.sourceStoryId"><dt>原聊天</dt><dd>{{ activity.sourceStoryId }}</dd></div></dl>
            </details>
        </div>
