...
const dialog = ref<HTMLDialogElement | null>(null); const mode = ref<'add' | 'detail' | 'delete' | 'sync' | 'recover'>('add');
...
async function readThread(older = false) {
    ...
    try {
        ...
        const previous = older || overlap ? page.value.messages : [];
        ...
        page.value = { ...next, messages, hasMore: older || !overlap || messages.length <= 50 ? next.hasMore : page.value.hasMore };
...
function apply(next: MessagesClientState) {
    ...
    else if (selected.value && previousSeq !== contact.value?.lastSeq) {void readThread();}
}
function clearSubmitted() {
    if (submitted) {
        ...
        if (current?.text === submitted.draft.text && current.type === submitted.draft.type) {drafts.delete(submitted.contactId);}
...
<template>
    <main class="messages-app">
        ...
        <dialog ref="dialog" class="messages-dialog" @keydown.esc.stop @click="event => { if (event.target === dialog) close(); }">
            <header><ContactAvatar v-if="mode === 'detail' && contact" :identity="contact.id" :name="contact.name" small /><h2>{{ mode === 'add' ? '新的对话' : mode === 'detail' ? contact?.name : mode === 'delete' ? '删除联系人？' : mode === 'sync' ? '消息还未写入主聊天' : '在当前位置补记？' }}</h2><button class="messages-icon-button" aria-label="关闭" @click="close"><MessageIcon name="close" /></button></header>
