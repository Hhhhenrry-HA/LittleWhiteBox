...
export function createMessagesService(store: PartitionStore<MessagesDomainV2>, files: XiaobaiOsFileControls) {
    ...
    async function change<T>(command: (state: MessagesDomainV2) => T, guard: () => boolean = () => true,
        recoveryGuard?: () => boolean): Promise<T> {
        const result = await store.transact(context => {
            ...
            return result;
        }, { commitGuard: guard, recoveryGuard, retainFailedCandidate: true });
