...
export type LearningLoopResult = { status: 'finished'; text: string }
...
export async function runLearningProviderLoop(options: {
    ...
    for (let round = 1; round <= MAX_LEARNING_ROUNDS; round++) {
        ...
        if ([...safePromptJson(messages)].length > MAX_LEARNING_CONTEXT) { return failure('learning_context_full'); }
        ...
        try {
            ...
            if (!calls.length) {
                ...
                return text ? { status: 'finished', text } : failure('learning_empty_response');
            }
            if (calls.length > 16) { return failure('learning_tool_limit'); }
            ...
            for (const call of calls) {
                ...
                if (!tools.has(call.name)) { return failure('learning_unknown_tool'); }
    ...
    return failure('learning_round_limit');
}
