...
const sceneLocation = computed(() => atlas.value?.locations.find(place => place.key === sceneKey.value));
const scene = computed(() => sceneLocation.value?.sceneKey ? state.value.map?.scenes[sceneLocation.value.sceneKey] : undefined);
...
watch(() => state.value, (next, previous) => {
    ...
    if (changedChat || !next.map?.atlas.locations.some(place => place.key === sceneKey.value && place.sceneKey)) {sceneKey.value = '';}
...
function handleEscape(event: KeyboardEvent): void {
    ...
    if (sceneKey.value || selectedKey.value || helpOpen.value) {
        event.stopPropagation();
        if (sceneKey.value) {sceneKey.value = '';}
...
<template>
    <main class="map-app" @keydown="handleEscape">
        <div class="map-top">
            <header class="map-search-bar"><MapIcon :name="scene ? 'layers' : 'search'" /><button v-if="!scene" type="button" :disabled="!atlas?.locations.length" @click="searchOpen = true">想去哪里？<small>搜索世界中的地点</small></button><button v-else type="button" @click="sceneKey = ''">{{ sceneLocation?.name }}<small>返回世界地图</small></button><button type="button" class="map-round-button" aria-label="地图设置" @click="settingsOpen = true"><MapIcon name="more" /></button></header>
            <nav v-if="atlas?.locations.length && !scene" class="map-region-trail" aria-label="当前查看区域"><button type="button" @click="enterRegion('')"><MapIcon name="globe" />世界</button><template v-for="place in trail" :key="place.key"><MapIcon name="next" /><button type="button" @click="enterRegion(place.key)">{{ place.name }}</button></template></nav>
        ...
        <div class="map-canvas" :class="{ 'has-detail': selected && !scene }">
            <template v-if="state.map && atlas?.locations.length">
                <MapAtlas v-show="!scene" :atlas="state.map.atlas" :region="region" :current-location-key="playerKey" :selected-location-key="selectedKey" :focus-key="focusKey" :focus-sequence="focusSequence" @select="key => selectPlace(key)" />
                <template v-if="scene"><MapScene v-if="scene.status === 'active'" :scene="scene" /><div v-else class="map-empty"><MapIcon name="layers" /><h2>{{ sceneLocation?.name }}</h2><p>这里的内部布局还没有画出来。<br>故事中的细节补充后，可以更新地图。</p><button type="button" class="map-secondary-button" @click="sceneKey = ''">返回世界地图</button></div></template>
                <div v-if="!scene && !places.length" class="map-empty"><MapIcon name="pin" /><h2>这里还没有标出更多地点</h2><p>可以先看看其他区域，或更新地图补充。</p><button type="button" class="map-secondary-button" @click="enterRegion(currentRegion?.parent || '')">查看上级区域</button></div>
            </template>
        ...
        <div v-if="atlas?.locations.length" class="map-floating-tools" :class="{ 'has-detail': selected && !scene }"><button v-if="scene" type="button" class="map-round-button" aria-label="返回世界地图" @click="sceneKey = ''"><MapIcon name="globe" /></button><button v-else type="button" class="map-round-button" :disabled="!player" aria-label="回到我的位置" @click="locatePlayer"><MapIcon name="locate" /></button><button type="button" class="map-round-button" :aria-expanded="helpOpen" aria-label="地图图例" @click="helpOpen = !helpOpen"><MapIcon name="layers" /></button></div>
        ...
        <MapPlaceDetail v-if="selected && state.map && !scene" :key="selected.key" :location="selected" :map="state.map" :current-key="playerKey" @close="selectedKey = ''" @scene="sceneKey = selected.key" @explore="enterRegion(selected.key)" @select="key => selectPlace(key, true)" />
        <footer v-else-if="atlas?.locations.length && !scene" class="map-region-card"><span class="map-region-icon"><MapIcon name="compass" /></span><div><h1>{{ currentRegion?.name || '世界地图' }}</h1><p>{{ places.length }} 个地点 · {{ unvisited ? unvisited + ' 处还没去过' : '看看熟悉的地方有什么变化' }}</p></div><button type="button" class="map-round-button" aria-label="浏览全部地点" @click="searchOpen = true"><MapIcon name="next" /></button></footer>
        <footer v-else-if="scene" class="map-scene-caption"><MapIcon name="layers" /><span><strong>{{ sceneLocation?.name }}</strong><small>{{ playerKey === sceneKey ? '当前位置的场景图' : '正在查看场景图 · 不会移动人物' }}</small></span></footer>
