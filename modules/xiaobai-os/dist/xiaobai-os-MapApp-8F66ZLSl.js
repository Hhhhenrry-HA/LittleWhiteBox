/* eslint-disable */
import { A as ve, B as ge, E as Te, F as K, L as Be, M as xe, O as he, R as p, S as se, T as Z, V as b, _ as Y, a as Ke, b as _e, c as O, d as W, f as k, g as S, h as N, i as qe, j as Ce, l as $, m as He, o as ke, p as r, s as G, u as n, w as l, y as ze, z as D } from "./xiaobai-os-runtime-dom.esm-bundler-DmE9neiz.js";
var Le = { class: "map-viewport" }, Ne = ["viewBox", "aria-label"], Ve = {
  ...
  setup(e) {
    const s = e, o = K(null), t = K([...s.viewBox]), u = K([0, 0]), v = $(() => u.value[0] && u.value[1] ? Math.max(t.value[2] / u.value[0], t.value[3] / u.value[1]) : 1);
    let f;
    se(() => {
    ...
    function w(m, j) {
      ...
      const H = T();
      return [t.value[0] + t.value[2] / 2 + (m - I.left - I.width / 2) * H, t.value[1] + t.value[3] / 2 + (j - I.top - I.height / 2) * H];
    }
    function g(m, j) {
      const I = Math.max(1, s.viewBox[2]), H = Math.min(I * 3, Math.max(Math.min(I * 0.24, 240), t.value[2] * m)), U = H / t.value[2], X = j || [t.value[0] + t.value[2] / 2, t.value[1] + t.value[3] / 2];
      t.value = [
        X[0] - (X[0] - t.value[0]) * U,
        X[1] - (X[1] - t.value[1]) * U,
        H,
    ...
    function P(m) {
      ...
      const j = [...i.values()];
      if (j.length === 2 && y) {
        const I = Math.hypot(j[1][0] - j[0][0], j[1][1] - j[0][1]), H = [(j[0][0] + j[1][0]) / 2, (j[0][1] + j[1][1]) / 2];
        I > 0 && a > 0 && g(a / I, w(...y)), t.value[0] -= (H[0] - y[0]) * T(), t.value[1] -= (H[1] - y[1]) * T(), a = I, y = H;
      } else if (c) {
        const I = m.clientX - c[0], H = m.clientY - c[1];
    ...
    return ve(() => s.resetKey, q, { immediate: !0 }), ve(() => s.focusSequence, V, { flush: "post" }), _e(() => {
      ...
      role: "group",
      "aria-label": e.label,
      onWheel: j[0] || (j[0] = G((I) => g(I.deltaY < 0 ? 0.84 : 1.19, w(I.clientX, I.clientY)), ["prevent"])),
      onPointerdown: z,
      onPointermove: P,
      onPointerup: A,
      onPointercancel: A,
...
var Xe = {
  ...
  emits: ["select"],
  setup(e) {
    const s = e, o = $(() => Ge(s.atlas, s.region)), t = $(() => ie(s.atlas, s.currentLocationKey, s.region)), u = $(() => o.value.nodes.find((i) => i.location.key === s.focusKey)), v = "map-arrow-" + he();
    function f(i, c) {
    ...
    return (i, c) => (l(), W(Se, {
      ...
      default: Ce(({ unitScale: d }) => [
        ...
        n("g", Xe, [(l(!0), r(O, null, Z(o.value.nodes, (a) => (l(), r("g", {
          key: a.location.key,
        ...
        (l(!0), r(O, null, Z(o.value.nodes, (a) => (l(), r("g", {
          ...
          onClick: G((y) => i.$emit("select", a.location.key), ["stop"]),
          onKeydown: [ke(G((y) => i.$emit("select", a.location.key), ["stop"]), ["enter"]), ke(G((y) => i.$emit("select", a.location.key), ["stop", "prevent"]), ["space"])]
        }, [
          c[2] || (c[2] = n("circle", {
            class: "map-pin-halo",
            r: "39"
...
function ye(e) {
  return ue(e).length >= 3 && (e.closed ?? qt.has(e.category));
}
function le(e) {
  return e.category === "wall" || e.category === "grid" ? !1 : e.shape === "rect" || e.shape === "circle" ? !0 : (e.shape === "path" || e.shape === "curve") && ye(e);
}
function Ee(e) {
  return (e.shape === "rect" || e.shape === "circle") && (e.icon !== void 0 && Ht.has(e.icon) || [
    "furniture",
    "decoration",
    "door"
  ].includes(e.category));
...
function we(e, s = 1) {
  ...
  if (e.shape === "path" || e.shape === "curve") {
    ...
    if (e.shape === "curve") {
      ...
      Q = A ** 3 * x[0] + 3 * A ** 2 * B * z[0] + 3 * A * B ** 2 * P[0] + B ** 3 * R[0], q = A ** 3 * x[1] + 3 * A ** 2 * B * z[1] + 3 * A * B ** 2 * P[1] + B ** 3 * R[1], T = 3 * A ** 2 * (z[0] - x[0]) + 6 * A * B * (P[0] - z[0]) + 3 * B ** 2 * (R[0] - P[0]), w = 3 * A ** 2 * (z[1] - x[1]) + 6 * A * B * (P[1] - z[1]) + 3 * B ** 2 * (R[1] - P[1]);
    }
    const g = Math.hypot(T, w);
    if (!g) return [Q, q - 13 * s];
    let V = -w / g, L = T / g;
    return (L > 0 || L === 0 && V < 0) && (V = -V, L = -L), [Q + V * 13 * s, q + L * 13 * s];
  }
...
var Zt = [
  ...
  setup(e) {
    ...
    return (d, a) => (l(), r("svg", {
      ...
      "clip-path": `url(#${c})`,
      fill: f.value
    }, [u.value ? (l(), r("circle", Ft)) : (l(), r("rect", Gt)), t.value ? (l(), r(O, { key: 2 }, [u.value ? (l(), r("circle", {
      key: 0,
      cx: "50",
      cy: "50",
      r: "44",
      ...
      d: "M11 7V93M89 7V93",
      class: "scene-object-shine"
    }, null, -1))], 64)) : e.element.icon === "tree" ? (l(), r(O, { key: 8 }, [a[14] || (a[14] = He('<circle cx="34" cy="32" r="24" class="scene-object-inset"></circle><circle cx="69" cy="36" r="24" class="scene-object-inset"></circle><circle cx="30" cy="62" r="23" class="scene-object-inset"></circle><circle cx="64" cy="67" r="25" class="scene-object-inset"></circle><circle cx="49" cy="48" r="26" class="scene-object-inset"></circle><path d="M21 24q10-10 22-4M36 41q8-9 22-6M64 56q8-1 13 5" class="scene-object-shine"></path>', 6))], 64)) : e.element.icon === "rock" ? (l(), r(O, { key: 9 }, [a[15] || (a[15] = n("path", {
...
var ha = ["data-element", "opacity"], fa = ["transform"], ya = ["d"], ma = ["d", "stroke-width"], ba = [
...
}), Aa = Oa, Ia = { class: "map-dialog-header" }, Pa = { key: 0 }, Ra = { class: "map-settings-content" }, Ta = { class: "map-auto-setting" }, Ba = ["aria-checked", "disabled"], Ka = { class: "map-settings-section" }, qa = ["disabled"], Ha = { key: 0 }, za = { class: "map-settings-section" }, La = { key: 0 }, Na = ["disabled"], Va = {
  key: 0,
  ...
  setup(e) {
    ...
    return se(() => s.value?.showModal()), (o, t) => (l(), r("dialog", {
      ...
      onCancel: t[5] || (t[5] = G((u) => o.$emit("close"), ["prevent"])),
      onKeydown: t[6] || (t[6] = G(() => {
      }, ["stop"]))
    }, [
      ...
      n("div", Ra, [
        ...
        n("section", Ka, [
          ...
          }, b(e.busy ? e.status || "请稍候…" : "更新地图"), 9, qa),
          e.hasMap ? k("", !0) : (l(), r("small", Ha, "请先建立世界地图"))
        ]),
        n("section", za, [
          S(E, { name: "globe" }),
...
}), Da = Qa, Ya = { class: "map-search-input" }, Fa = {
  class: "map-search-filters",
  "aria-label": "地点筛选"
}, Ga = ["aria-pressed", "onClick"], Xa = { class: "map-search-results" }, Wa = ["onClick"], Ua = { class: "map-result-icon" }, Ja = { key: 0 }, en = {
  key: 0,
  ...
  emits: ["close", "select"],
  setup(e) {
    const s = e, o = K(null), t = K(""), u = K("all"), v = $(() => s.atlas.locations.filter((f) => [
      f.name,
      f.brief,
    ...
    return se(() => o.value?.showModal()), (f, i) => (l(), r("dialog", {
      ...
      "aria-label": "查找地点",
      onCancel: i[2] || (i[2] = G((c) => f.$emit("close"), ["prevent"])),
      onKeydown: i[3] || (i[3] = G(() => {
      }, ["stop"]))
    }, [
      n("header", Ya, [
  ...
  setup(e) {
    const s = e, o = $(() => fe(s.map.atlas, s.location.key).slice(0, -1)), t = $(() => s.map.atlas.locations.filter((i) => i.parent === s.location.key)), u = $(() => s.map.atlas.actors.filter((i) => i.locationKey === s.location.key)), v = $(() => Fe(s.map.atlas, s.location.key)), f = $(() => s.location.sceneKey ? s.map.scenes[s.location.sceneKey] : void 0);
...
function mn(e) {
  const s = K(structuredClone(Be(e.initialState))), o = K(null), t = K(""), u = K(!1);
  let v = !1, f = 0, i = 0, c = () => {
  };
  ...
  async function T(w, g, V = {}) {
    ...
    try {
      ...
      v && L === f && s.value.chatIdentity === P && (t.value = q(A, g), u.value = !0);
    } finally {
      v && L === f && (o.value = null);
    }
  }
...
var bn = { class: "map-top" }, gn = { class: "map-search-bar" }, kn = ["disabled"], Mn = {
...
}, Bn = { class: "map-empty-art" }, Kn = ["disabled"], qn = {
  key: 1,
  class: "map-setting-note"
}, Hn = ["disabled"], zn = ["aria-expanded"], Ln = {
  key: 1,
  class: "map-key"
}, Nn = {
  key: 3,
  ...
  setup(e) {
    const { state: s, activeRequest: o, busy: t, disabledReason: u, requiresConfirmation: v, status: f, notice: i, isError: c, dismissNotice: d, refresh: a, confirmSave: y, adopt: x, setAuto: R, update: B, rebuild: Q } = mn(e), q = K(s.value.map ? Me(s.value.map.atlas) : ""), T = K(""), w = K(null), g = $(() => w.value !== null), V = K(""), L = K(0), z = K(!1), P = K(!1), A = K(!1), _ = $(() => s.value.map?.atlas), m = $(() => _.value?.actors.find((C) => C.actorKey === "player")?.locationKey || ""), j = $(() => _.value?.locations.find((C) => C.key === m.value)), I = $(() => _.value?.locations.find((C) => C.key === T.value)), H = $(() => _.value?.locations.find((C) => C.key === (w.value || m.value))), U = $(() => g.value && H.value?.sceneKey ? s.value.map?.scenes[H.value.sceneKey] : void 0), X = $(() => _.value?.locations.find((C) => C.key === q.value)), ce = $(() => _.value?.locations.filter((C) => (C.parent || "") === q.value) || []), me = $(() => ce.value.filter((C) => C.status !== "visited").length), Ie = $(() => _.value ? fe(_.value, q.value) : []);
    ve(() => s.value, (C, h) => {
      const M = C.chatIdentity !== h.chatIdentity;
      (!h.map || M || q.value && !C.map?.atlas.locations.some((F) => F.key === q.value)) && (q.value = C.map ? Me(C.map.atlas) : ""), (M || !C.map?.atlas.locations.some((F) => F.key === T.value)) && (T.value = ""), (M || w.value && !C.map?.atlas.locations.some((F) => F.key === w.value)) && (w.value = null), M && (z.value = !1, P.value = !1);
    });
    function oe(C) {
      q.value = C, T.value = "", w.value = null, A.value = !1;
    }
    async function re(C, h = !1) {
      const M = _.value?.locations.find((F) => F.key === C);
      M && (w.value = null, T.value = C, P.value = !1, A.value = !1, h && (q.value = M.parent || ""), await ze(), V.value = _.value ? ie(_.value, C, q.value) : C, L.value += 1);
    }
    async function Pe() {
    ...
    return (C, h) => (l(), r("main", {
      ...
      n("div", bn, [
        n("header", gn, [
          ...
          g.value ? (l(), r("div", Mn, [N(b(H.value?.name || "当前场景"), 1), n("small", null, b(w.value ? "正在查看已记录的场景" : "看看你身边的布局"), 1)])) : (l(), r("button", {
            key: 0,
            type: "button",
            class: "map-search-entry",
      ...
      n("div", { class: D(["map-canvas", { "has-detail": I.value && !g.value }]) }, [p(s).map && _.value?.locations.length ? (l(), r(O, { key: 0 }, [
        ...
        g.value ? (l(), r(O, { key: 0 }, [U.value?.status === "active" ? (l(), W(Aa, {
          ...
          S(E, { name: "layers" }),
          n("h2", null, b(H.value ? "这里的布局还没画出来" : "还不知道你在哪里"), 1),
          n("p", null, b(H.value ? "更新地图后，会结合设定与剧情补齐这里的普通布局。" : "更新地图后，会根据剧情确认你所在的地方。"), 1),
          n("button", {
            type: "button",
            class: "map-secondary-button",
            disabled: !!p(u),
        ...
        !g.value && !ce.value.length ? (l(), r("div", Rn, [
          ...
          n("button", {
            type: "button",
            class: "map-secondary-button",
            onClick: h[10] || (h[10] = (M) => oe(X.value?.parent || ""))
          }, "查看上级区域")
        ])) : k("", !0)
      ], 64)) : (l(), r("div", Tn, [
        ...
        }, b(p(t) ? p(f) || "正在准备…" : "绘制世界地图"), 9, Kn)) : k("", !0),
        p(u) && !p(t) ? (l(), r("p", qn, b(p(u)), 1)) : k("", !0)
      ]))], 2),
      ...
      I.value && p(s).map && !g.value ? (l(), W(yn, {
        ...
        n("div", null, [n("h1", null, b(X.value?.name || "世界地图"), 1), n("p", null, b(ce.value.length) + " 个地点 · " + b(me.value ? me.value + " 处还没去过" : "看看熟悉的地方有什么变化"), 1)]),
        n("button", {
          type: "button",
          class: "map-round-button",
          "aria-label": "浏览全部地点",
      ...
      ])) : g.value && _.value?.locations.length ? (l(), r("footer", Zn, [S(E, { name: "layers" }), n("span", null, [n("strong", null, b(H.value?.name || "当前位置待确认"), 1), n("small", null, b(w.value ? "正在查看场景图 · 不会移动人物" : "当前位置的场景图"), 1)])])) : k("", !0),
