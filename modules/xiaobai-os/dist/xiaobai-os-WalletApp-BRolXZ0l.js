/* eslint-disable */
import { E as D, F as w, L as P, R as $, S as z, T as E, V as r, _ as b, b as X, c as T, d as B, f as p, g, h as S, j as F, l as _, o as Y, p as s, s as A, u as e, w as n, z as C } from "./xiaobai-os-runtime-dom.esm-bundler-DmE9neiz.js";
var ee = {
  class: "wallet-icon",
  viewBox: "0 0 24 24",
  fill: "none",
  ...
  "aria-hidden": "true"
}, pe = { class: "wallet-ui-notice-copy" }, ge = { key: 0 }, we = /* @__PURE__ */ b({
  __name: "WalletNotice",
  props: {
    title: {},
    message: { default: "" },
  ...
  setup(t) {
    return (i, a) => (n(), s("aside", {
      ...
      role: "status"
    }, [e("span", he, [D(i.$slots, "icon", {}, () => [a[0] || (a[0] = S("!", -1))])]), e("div", pe, [
...
var Ce = {
  ...
  role: "status"
}, Ee = { class: "wallet-ui-list" }, Ae = { class: "wallet-ledger-foot" }, De = {
  key: 0,
  ...
  setup(t) {
    ...
    return (h, m) => (n(), s("div", null, [
      e("div", xe, [(n(), s(T, null, E(c, (f) => e("button", {
        key: f.id,
        type: "button",
        "aria-pressed": a.value === f.id,
        onClick: (u) => a.value = f.id
      }, r(f.label), 9, Ne)), 64))]),
      a.value === "transfer" ? (n(), s("p", Ve, "系统账户间的划转，不计入你的个人收支。")) : p("", !0),
      t.loading ? (n(), s("div", Le, [g(y, {
        name: "refresh",
        class: "is-spinning"
        ...
        l.value.length ? p("", !0) : (n(), B(Me, {
          ...
          icon: F(() => [g(y, { name: "receipt" })]),
          _: 1
        }, 8, ["title"])),
        (n(!0), s(T, null, E(l.value, (f) => (n(), s("section", {
          key: f.transactions[0].id,
          class: "wallet-day-group"
        }, [e("h3", null, r(f.date), 1), e("ol", Ee, [(n(!0), s(T, null, E(f.transactions, (u) => (n(), B(Se, {
          key: u.id,
          transaction: u,
        ...
        }, [S(r(t.loadingMore ? "正在读取…" : "查看更早的账单"), 1), g(y, { name: "next" })], 8, Ie)) : t.transactions.length ? (n(), s("span", He, "每一笔，都有来处")) : p("", !0)])
      ], 64))
    ]));
  }
  ...
  setup(t) {
    ...
    return (v, l) => (n(), s("dialog", {
      ...
      "aria-label": "账单详情",
      onCancel: l[1] || (l[1] = A((h) => v.$emit("close"), ["prevent"])),
      onKeydown: [l[2] || (l[2] = A(() => {
      }, ["stop"])), l[3] || (l[3] = Y(A((h) => a.value?.focus(), ["prevent"]), ["tab"]))]
    }, [
      ...
      e("div", { class: C(["wallet-receipt-hero", `is-${t.transaction.direction}`]) }, [
        e("span", Fe, [g(y, { name: $(O)(t.transaction) }, null, 8, ["name"])]),
        e("h2", null, r(t.transaction.title), 1),
        e("strong", null, [S(r($(Z)(t.transaction)), 1), l[5] || (l[5] = e("small", null, "小白币", -1))]),
...
}), Ue = Oe, je = { class: "wallet-ui-app wallet-app" }, Ke = { class: "wallet-ui-scroll" }, Ge = ["disabled"], Qe = ["disabled"], Je = {
  class: "wallet-ledger",
  ...
  setup(t) {
    const i = t, a = w(structuredClone(P(i.initialState))), c = w(!1), v = w(!1), l = w(""), h = w(""), m = w(null);
    let f = () => {
    }, u = 0;
    const M = _(() => a.value.status === "unconfirmed"), W = _(() => c.value || a.value.status === "loading" || a.value.status === "saving"), I = _(() => W.value || M.value || a.value.status === "conflict"), U = _(() => !!(a.value.message || l.value)), j = _(() => l.value || a.value.status === "conflict" || a.value.status === "blocked" ? "danger" : M.value ? "warning" : "info"), K = _(() => a.value.status === "conflict" ? "账本发生冲突" : a.value.status === "blocked" ? "钱包暂时无法读取" : "账本状态");
    function x(o) {
    ...
    async function Q() {
      ...
      try {
        ...
        if (d !== u) return;
        const J = new Set(a.value.transactions.map((L) => L.id));
        a.value.transactions.push(...k.result.transactions.filter((L) => !J.has(L.id))), a.value.nextCursor = k.result.nextCursor, a.value.hasMore = k.result.hasMore;
      } catch {
    ...
    return z(() => {
      f = i.bridge.subscribe((o) => {
        o.type === "wallet/state" && (u += 1, V(o.payload.state)), o.type === "wallet/error" && (l.value = x(o.payload?.message || ""));
      });
    }), X(() => {
      u += 1, f();
    }), (o, d) => (n(), s("main", je, [
